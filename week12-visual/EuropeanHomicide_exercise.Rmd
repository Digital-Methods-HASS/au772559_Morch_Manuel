...
*The data is pretty tidy, but the dataset is not very clean because of a long column name, and one column needing a new name.*

Do you understand what the column names represent? 

*Yes, but the "Entity" column has to be renamed.*

What is the difference between rate and homicide number?

*The difference between rate and homicide number, is that rate is the number for homicides per 100,000 individuals, while the homicide number is the total number of homicides.*
...
Western_Europe %>% 
  ...
  geom_line(lwd = 2) + 
  theme_classic() +
  labs(title = "Historical homicides",
       x = "Year",
...
ggsave("homcides2.png", width = 16, height = 8, dpi = 150)
```
...
*Done, but i just made the "midyear" and "duration" columns again*
...
*The Monarchs seem to be ruling for a longer and longer time. At the same time, the amount of homicides are falling. This is a sign of stability*

```{r plotting_monarchs, eval = TRUE, warning=FALSE}

kings <- read_csv("data/monarchs.csv")
...
*I already labeled it "Country" in the code above, but here is how you would change the legend title if it was not already done:*
...
- Yes we are. The steady decline in homicide shows this very clearly. The fact that the monarchs are ruling for longer and longer time shows that society has become more and more stable through time.